...
F.magic = ProtoField.uint16("prudpv1.magic", "Magic", base.HEX)
F.ver = ProtoField.uint8("prudpv1.ver", "Version", base.HEX)
F.len = ProtoField.uint8("prudpv1.len", "Packet Length", base.HEX)
F.psiz = ProtoField.uint16("prudpv1.psiz", "Payload Size", base.Hex)
F.src = ProtoField.uint8("prudpv1.src", "Source", base.HEX, ports)
F.dst = ProtoField.uint8("prudpv1.dst", "Destination", base.HEX, ports)
F.type = ProtoField.uint16("prudpv1v1.type", "Type", base.HEX, pkt_types, 0xf) -- Low nibble of the Type & Flags short
...
F.ack_ver = ProtoField.uint8("prudpv1.ack_ver", "Multi-ack version", base.HEX)
F.seq = ProtoField.uint16("prudpv1.seq", "Sequence number", base.HEX)

F.packet_sig = ProtoField.ubytes("prudpv1.packet_sig", "Packet signature", base.HEX)

F.conn_sig = ProtoField.uint32("prudpv1.conn_sig", "Connection signature", base.HEX)
F.frag = ProtoField.uint8("prudpv1.frag", "Fragment", base.HEX)
F.size = ProtoField.uint16("prudpv1.size", "Packet size", base.HEX)

F.payload = ProtoField.bytes("prudpv1.payload", "Payload")
F.checksum = ProtoField.uint8("prudpv1.checksum", "Checksum", base.HEX)
...
function prudp_v1_proto.dissector(buf,pinfo,tree)
	pinfo.cols.protocol = "PRUDPv1"
	-- Parse the packet header.

	local subtree = tree:add(prudp_v1_proto, buf(), "PRUDPv1")
	
	local payload_size = nil

	local pkt_magic = buf(0, 2):le_uint()
	subtree:add(F.magic, buff(0, 2))
	
	local pkt_ver = buff(2, 1):le_uint()
	subtree:add(F.ver, buff(2, 1))
	
	local pkt_len = buff(3, 1):le_uint()
	subtree:add(F.len, buff(3, 1))
	
	local pkt_psiz = buff(4, 2):le_uint()
	subtree:add(F.psiz, buff(4, 2))
	
	local pkt_src = buf(6,1):le_uint()
	subtree:add(F.src, buf(6,1))

	local pkt_dst = buf(7,1):le_uint()
	...
	local pkt_session_id = buf(10,1):le_uint()
	subtree:add(F.session_id, buf(10,1))

	local pkt_ack_ver = buff(11, 1):le_uint()
	subtree:add_le(F.ack_ver, buff(11, 1))
	
	local pkt_seq = buf(12,2)
	subtree:add_le(F.seq, buf(12, 2))

	subtree:add(F.packet_sig, buf(14,16))
	
	-- Packet specific. This needs to be updated for v1.
	off = 11
...
udp_table:add(60000, prudp_v1_proto)